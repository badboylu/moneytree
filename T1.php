...
Email.send({
    ...
    Body : '<p>Hello <?php echo $username; ?>,</p><p>you have successfully <span style="color:green">placed</span> an order at <span style="color:orange">canibuy</span>.</p><p><h3>Transaction details:</h3></p><p>Amount transacted: R<?php echo $total; ?></p><p><h3>Order:</h3></p><p><?php if($dp>0){echo'Durban poison:';} ?> <?php if($dp>0){echo $dp;} ?>gram/s</p><p><?php if($cc>0){echo'Dabcapsules (Light):';} ?> <?php if($cc>0){echo $cc;} ?>batch/es</p><p><?php if($oc>0){echo'Orange cali:';} ?> <?php if($oc>0){echo $oc;} ?>gram/s</p><p><?php if($bw>0){echo'Raw rizzla:';} ?> <?php if($bw>0){echo $bw;} ?>pack/s</p><p><?php if($pr>0){echo'Pre-roll:';} ?> <?php if($pr>0){echo $pr;} ?>J/'s</p><p><?php if($nl>0){echo'Northern lights:';} ?> <?php if($nl>0){echo $nl;} ?>gram/s</p><p><?php if($cces>0){echo'Dabcapsules (Strong):';} ?> <?php if($cces>0){echo $cces;} ?>batch/es</p><p><?php if($pe>0){echo'Pineapple express:';} ?> <?php if($pe>0){echo $pe;} ?>gram/s</p> '              
  });
 }
}
sendemail();
