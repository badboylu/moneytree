...
Email.send({
    ...
    Body : '<p>Hello <?php echo $username; ?>,</p><p>you have successfully <span style="color:green">placed</span> an order at <span style="color:orange">canibuy</span>.</p><p><h3>Transaction details:</h3></p><p>Amount transacted:<span> R</span><?php echo $total; ?><span>.00</span></p><p><h3>Order:</h3></p><p><?php if($dp>0){echo'Durban poison:';} ?><span> </span><?php if($dp>0){echo $dp;} ?><span> gram/s</span></p><p><?php if($cc>0){echo'Dabcapsules (Light):';} ?><span> </span><?php if($cc>0){echo $cc;} ?><span> batch/es</span></p><p><?php if($oc>0){echo'Orange cali:';} ?><span> </span><?php if($oc>0){echo $oc;} ?><span> gram/s</span></p><p><?php if($bw>0){echo'Raw rizzla:';} ?><span> </span><?php if($bw>0){echo $bw;} ?><span> pack/s</span></p><p><?php if($pr>0){echo'Pre-roll:';} ?><?php if($pr>0){echo $pr;} ?></p><p><?php if($nl>0){echo'Northern lights:';} ?><?php if($nl>0){echo $nl;} ?></p><p><?php if($cces>0){echo'Dabcapsules (Strong):';} ?><?php if($cces>0){echo $cces;} ?></p><p><?php if($pe>0){echo'Pineapple express:';} ?><?php if($pe>0){echo $pe;} ?></p>'              
  });
 }
}
sendemail();
